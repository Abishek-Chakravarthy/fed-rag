    ...
    for row in final_rows:
        ...
        bucket = summary_by_alpha.setdefault(
            alpha,
            {
                ...
                "retriever_model": row.get("retriever_model", ""),
                "seed_count": 0,
                "avg_loss": [],
                "avg_train_loss": [],
                "server_val_mrr": [],
        ...
        summary_rows.append(
            {
                ...
                "benchmark_mode": str(bucket["benchmark_mode"]),
                "noise_mode": str(bucket["noise_mode"]),
                "noise_ratio": str(bucket["noise_ratio"]),
                "retriever_model": str(bucket["retriever_model"]),
...
def plot_final_loss(summary_rows: list[dict], output_dir: Path) -> Path:
    ...
    plt.xlabel("Alpha")
    plt.ylabel("Final Shared Quality Loss")
    plt.title("Mechanism Benchmark: Alpha vs Final Shared Quality Loss")
...
def plot_post_metrics(summary_rows: list[dict], output_dir: Path) -> list[Path]:
    ...
    for mean_key, std_key, label, filename in metric_specs:
        ...
        plt.xlabel("Alpha")
        plt.ylabel(label)
        plt.title(f"Mechanism Benchmark: Alpha vs {label}")
        plt.grid(True, linestyle="--", alpha=0.4)
...
def plot_round_curves(round_rows: list[dict], output_dir: Path) -> Path:
    ...
    plt.title("Mechanism Benchmark: Server-Validation MRR by Alpha")
    plt.grid(True, linestyle="--", alpha=0.4)
    plt.legend()
